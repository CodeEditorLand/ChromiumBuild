...
function showSuiteTable(show_the_table) {
	document.getElementById("suite-table").style.display = show_the_table
		? "table"
		: "none";
}

function showTestTable(show_the_table) {
	document.getElementById("test-table").style.display = show_the_table
		? "table"
		: "none";
}
...
function showTestsOfOneSuiteOnly(suite_name) {
	...
	var testTableBlocks = document
		.getElementById("test-table")
		.getElementsByClassName("row_block");
	Array.prototype.slice
		.call(testTableBlocks)
		.forEach(function (testTableBlock) {
			if (!show_all) {
				var table_block_in_suite =
					testTableBlock.firstElementChild.firstElementChild.firstElementChild.innerHTML.startsWith(
						suite_name,
					);
...
function sortByColumn(head) {
	...
	for (var i = 0; i < headers.length; i++) {
		if (headers[i].dataset.ascSorted != 0) {
			if (headers[i].dataset.ascSorted == 1) {
				headers[i].getElementsByClassName("up")[0].style.display =
					"none";
			} else {
				headers[i].getElementsByClassName("down")[0].style.display =
					"none";
			}
			if (headers[i] == head) {
	...
	if (asc == 1) {
		headers[headIndex].getElementsByClassName("up")[0].style.display =
			"inline";
	} else {
		headers[headIndex].getElementsByClassName("down")[0].style.display =
			"inline";
	}
	...
	rowBlocks.sort(function (a, b) {
		...
		if (head.className == "text") {
			...
			return aInnerHTML == bInnerHTML
				? 0
				: aInnerHTML > bInnerHTML
					? asc
					: -1 * asc;
			...
			var avalue =
				atotal_minus_skipped == 0
					? -1
					: (a_rows.length - a_success_or_skipped) /
						atotal_minus_skipped;
			var bvalue =
				btotal_minus_skipped == 0
					? -1
					: (b_rows.length - b_success_or_skipped) /
